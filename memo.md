...
## パッケージノードが重複表示される問題の修正（2025-04-05）

### 問題

テストエクスプローラーにパッケージ名のノード（"server"）が 2 つ表示されていた。

### 原因

`TestTreeDataProvider` クラスの `addPackageTestNode` メソッドで、既存のパッケージノードと同名のノードが存在するかチェックせずに常に新しいノードを追加していたため。

### 対応

`addPackageTestNode` メソッドを修正し、同名のパッケージノードが既に存在する場合は追加しないようにしました。

具体的には以下の変更を実施:

1. rootNodes の中に同じ名前とタイプのノードが存在するか確認
2. 既存ノードがある場合は新しいノードを追加しない

これにより、同名のパッケージノードが重複して表示される問題を解決しました。
...
1. `package.json`からテスト結果ビューの設定を削除
2. `src/testResultView.ts`ファイルを削除
3. `src/debugger.ts`からテスト出力イベントエミッターとテスト出力収集機能を削除
4. `src/extension.ts`からテスト結果表示関連のコードを削除

これにより、不要なテスト結果パネルが表示されなくなり、拡張機能がよりシンプルになりました。

## 2024-07-22: ARCHITECTURE.md の更新履歴を本文に統合

### 対応内容

- ARCHITECTURE.md にあった「最近の改善点」セクションを削除し、その内容を関連する常設セクションに統合
- 各改善点を適切なセクションに反映させ、ドキュメント全体の一貫性を向上

### 更新内容

1. **主な機能**セクションを更新:

   - ショートカットキー情報を追加（Cmd+Shift+D/Ctrl+Shift+D）
   - モノレポ環境における「{パッケージ名}のテストを実行」機能を追加
   - 「ユニットテストを実行」「E2E テストを実行」などの簡潔なテスト実行オプションを追加

2. **各コンポーネント説明**を拡充:

   - `extension.ts`: ディレクトリやファイル切り替え時の表示管理について追加
   - `debugger.ts`: `--testPathPattern`オプションの使用や正規表現処理の強化について追加
   - `testTreeDataProvider.ts`: モノレポ環境でのパッケージノード自動生成について追加
   - `monorepoDetector.ts`: Jest 設定ファイル検出機能について追加

3. **データフロー**セクションを更新:

   - テスト結果が統合ターミナルに直接表示される点を明記

4. **実装の注意点**セクションに追加:

   - サブディレクトリ除外に関する注意点を追加

5. **セクション番号の修正**:
   - 「最近の改善点」セクションを削除したことに伴い、後続のセクション番号を更新

この更新により、ARCHITECTURE.md がより一貫性のある最新のドキュメントとなり、改善点が各機能の説明に適切に統合されました。これにより、新しい開発者がプロジェクトを理解しやすくなりました。
