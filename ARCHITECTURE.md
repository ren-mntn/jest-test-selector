...
- 正確なテスト階層構造の認識と表示
- 現在のディレクトリ内のすべてのテストをワンクリックで実行する機能
...
│   ├── testResultView.ts   # テスト結果表示用WebView
...
- テスト結果ビューとテスト設定ビューのプロバイダーを登録
- テストツリービューのデータプロバイダーを初期化・登録
- テスト出力とテストセッション終了イベントの購読
- 以下のコマンドの登録と実装:
...
- テスト実行結果を表示する WebView の管理
- シングルトンパターンでのインスタンス管理
- 以下の機能を提供:
  - ANSI カラーコードを HTML/CSS に変換してリッチな出力表示
  - テスト実行中の状態表示（ローディングインジケータ）
  - テスト結果の整形と表示
  - エラーメッセージの表示
- `TestResultProvider` クラスによる WebView プロバイダー実装
- ビューの状態管理（実行中、完了、エラー、初期表示）
- VS Code のテーマに適応するスタイリング
...
- **テスト結果の詳細表示**:
  `testResultView.ts` をカスタマイズしてより詳細な情報を表示
...
- **「All Tests」オプション**:
  ファイル内のすべてのテストを一括実行するオプションを追加

- **実装ファイルからのテスト表示**:
  実装ファイルからテストファイルを自動検出して表示

- **ディレクトリベースのテスト表示**:
  ディレクトリ内のすべてのテストファイルを一元的に表示・管理

- **テスト抽出ロジックの改善**:
  括弧の深さ追跡による正確なテスト階層構造の認識

- **現在のディレクトリテスト実行**:
  現在フォーカスしているファイルが存在するディレクトリ内のすべてのテストを一括実行する機能を追加
  ショートカットキー（Cmd+Shift+D/Ctrl+Shift+D）でも実行可能

- **ディレクトリテスト実行の精度向上**:
  ディレクトリ内のテスト実行時にサブディレクトリ内のテストを除外する機能を追加
  `--testPathPattern` オプションを使用して指定ディレクトリ直下のテストファイルのみが実行されるように改善
  正規表現パターンのエスケープ処理を強化し、様々な環境で一貫して動作するように対応

- **ディレクトリテスト実行オプションの改善**:
  ディレクトリのテスト実行オプションを「すべてのテストを実行」「ユニットテストを実行」「E2E テストを実行」に簡潔化し、
  ユーザーエクスペリエンスを向上

- **「ワークスペース全体のテストを実行」ノードの表示修正**:
  テストツリーの更新時にワークスペース全体のテストを実行ノードが消えてしまう問題を修正
  ディレクトリやファイルの切り替え時にもトップレベルのノードが常に表示されるように改善

- **「パッケージのテストを実行」ノードの追加**:
  モノレポ環境において、現在のファイルパスから Jest 設定ファイル（jest.config.js）のあるディレクトリを自動的に検出し、
  そのパッケージ名を使用して「{パッケージ名}のテストを実行」ノードをテストエクスプローラーに追加
  クリックすると、検出されたパッケージディレクトリ配下の全テストを実行可能
  例えば `/Users/ren/workspace/aeon-pet/apps/server/src/routes/app/...` のファイルを開いている場合、
  「server のテストを実行」というノードが表示され、`/Users/ren/workspace/aeon-pet/apps/server/` 内の全テストを実行
