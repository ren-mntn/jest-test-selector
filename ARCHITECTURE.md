...
- テスト実行時のターミナル出力キャプチャ
- テスト結果の履歴管理機能
  - `.vscode/.jest-test-selector-history.json`ファイルにテスト結果を永続的に保存
  - 拡張機能起動時に自動的に履歴を読み込み、VSCode 再起動後もテスト結果を表示
  - 古いテスト結果（30 日以上）の自動クリーンアップ
  - 一時ファイルの自動削除によるディスク容量の最適化
- 以下の主要な機能を提供:
  ...
  - `loadHistoryFile`: 履歴ファイルからテスト結果を読み込む
  - `saveTestResultsToHistory`: 現在のテスト結果を履歴ファイルに保存
  - `cleanupOldResults`: 古いテスト結果をクリーンアップ
